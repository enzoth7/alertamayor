...
export function normalizeText(value) {
  return String(value ?? "")
    ...
    .replace(/\b(avenida|avda|av|bvar|bulevar|boulevard)\b/g, " av ")
...
function normalizedSet(values, normalizer) {
...
function contactValues(row, singular, plural) {
  return [
    ...scalarValues(row?.[singular]),
    ...scalarValues(row?.[plural]),
  ];
}
...
function normalizePublicUrl(value) {
  ...
  try {
    ...
    parsed.hash = "";
    parsed.search = "";
    return `${parsed.hostname.replace(/^www\./, "").toLocaleLowerCase("en-US")}${parsed.pathname.replace(/\/+$/, "")}`;
  } catch {
    return "";
  }
}
...
export function normalizeAddress(value, row = {}) {
  ...
  const numericTokenCount = rawTokens.filter((token) => /^\d{1,6}[a-z]?$/.test(token)).length;
  const tokens = rawTokens.filter((token) =>
    !(/^\d{5}$/.test(token) && numericTokenCount > 1),
  );

  // “Uruguay” es también una calle frecuente. Solo se descarta como país
  // cuando cierra una dirección que ya contiene otros componentes.
  if (tokens.length > 2 && tokens.at(-1) === "uruguay") tokens.pop();
...
export function addressParts(value, row = {}) {
  ...
  const plausibleDoorIndexes = numericIndexes.filter((index) =>
    !/^\d{5}$/.test(tokens[index]) || numericIndexes.length === 1,
  );
  const doorIndex = plausibleDoorIndexes.at(-1) ?? numericIndexes.at(-1);
...
export function scoreFacilityMatch(candidate, existing) {
  ...
  const candidatePhones = normalizedSet(contactValues(candidate, "phone", "phones"), normalizePhone);
  const existingPhones = normalizedSet(contactValues(existing, "phone", "phones"), normalizePhone);
  const phoneAvailable = candidatePhones.size > 0 && existingPhones.size > 0;
  const phoneExact = phoneAvailable && setsOverlap(candidatePhones, existingPhones);
  const phoneConflict = phoneAvailable && !phoneExact;

  const candidateEmails = normalizedSet(contactValues(candidate, "email", "emails"), normalizeEmail);
  const existingEmails = normalizedSet(contactValues(existing, "email", "emails"), normalizeEmail);
  const emailAvailable = candidateEmails.size > 0 && existingEmails.size > 0;
  const emailExact = emailAvailable && setsOverlap(candidateEmails, existingEmails);

  const candidateDomains = normalizedSet(contactValues(candidate, "domain", "domains"), normalizeDomain);
  const existingDomains = normalizedSet(contactValues(existing, "domain", "domains"), normalizeDomain);
  const domainAvailable = candidateDomains.size > 0 && existingDomains.size > 0;
  const domainExact = domainAvailable && setsOverlap(candidateDomains, existingDomains);

  const candidateSocialUrls = normalizedSet(scalarValues(candidate.socialUrls), normalizePublicUrl);
  const existingSocialUrls = normalizedSet(scalarValues(existing.socialUrls), normalizePublicUrl);
  const socialUrlAvailable = candidateSocialUrls.size > 0 && existingSocialUrls.size > 0;
  const socialUrlExact = socialUrlAvailable && setsOverlap(candidateSocialUrls, existingSocialUrls);

  const candidateExternalIds = normalizedSet(scalarValues(candidate.externalIds), normalizeText);
  const existingExternalIds = normalizedSet(scalarValues(existing.externalIds), normalizeText);
  const externalIdAvailable = candidateExternalIds.size > 0 && existingExternalIds.size > 0;
  const externalIdExact = externalIdAvailable && setsOverlap(candidateExternalIds, existingExternalIds);
  ...
  const streetScore = addressAvailable
    ...
    : 0;
  const streetContainment = addressAvailable && tokenSubset(candidateAddress.street, existingAddress.street);
  ...
  let score = weightedAverage([
    ...
    { available: emailAvailable, value: emailExact ? 1 : 0, weight: 0.35 },
    { available: domainAvailable, value: domainExact ? 1 : 0, weight: 0.25 },
    { available: socialUrlAvailable, value: socialUrlExact ? 1 : 0, weight: 0.25 },
    { available: externalIdAvailable, value: externalIdExact ? 1 : 0, weight: 0.4 },
  ...
  const hasPartialStreetIdentity = Boolean(
    streetContainment &&
      name.score === 1 &&
      departmentMatch &&
      localityScore >= 0.8 &&
  ...
  const hasStrongIdentity = phoneExact || emailExact || domainExact || socialUrlExact || externalIdExact
    || hasAddressIdentity || hasPartialStreetIdentity || hasGeoIdentity;
...
export function classifyFacilityMatch(bestMatch) {
  ...
  const corroborated =
    ...
    bestMatch.emailExact ||
    bestMatch.domainExact ||
    bestMatch.socialUrlExact ||
    bestMatch.externalIdExact ||
    bestMatch.hasAddressIdentity ||
    bestMatch.hasPartialStreetIdentity ||
  ...
  if ((bestMatch.score >= 0.88 || bestMatch.hasAddressIdentity || bestMatch.hasPartialStreetIdentity) && corroborated && !conflicting) {
