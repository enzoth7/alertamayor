...
const concerns = ["Faltan cuidados o apoyos", "Maltrato o trato inadecuado", "Riesgo para su salud o seguridad", "Problema con un servicio o residencial", "Otra preocupación"];
...
export function IntakeReportForm({ onHome }: { onHome: () => void }) {
  ...
  const validate = (): boolean => {
    if (step === 1 && (!draft.setting || !draft.concerns.length || draft.narrative.trim().length < 10)) {
      setMessage("Elegí el ámbito, al menos una preocupación y contá brevemente qué está pasando.");
      return false;
    }
  ...
  if (caseCode) return <section className="reportFlow reportSuccess">
    ...
    <p>El código de seguimiento es <strong>{caseCode}</strong>. El equipo puede verla en su bandeja de entradas de demostración.</p>
    <div className="reportSuccessActions"><button className="reportBack" onClick={restart}>Nueva comunicación</button><button className="reportContinue" onClick={onHome}>Volver al inicio <ArrowRight size={17}/></button></div>
  </section>;
  ...
  return <section className="reportFlow">
    <header className="reportFlowHeader">
      ...
      <h1>{stageTitle}</h1>
      <p className="lead">Completá sólo lo necesario. Los campos de cada paso son obligatorios para poder enviar.</p>
    </header>
    ...
    <div className="reportStage">
      {step === 1 && <>
        <p className="reportStageHelp">Elegí la situación que más se parece a lo que querés comunicar.</p>
        ...
        <label className="reportField"><span>Contá brevemente qué está pasando</span><textarea value={draft.narrative} onChange={(event) => update("narrative", event.target.value)} placeholder="Escribí un resumen breve. Usá sólo datos de demostración." /></label>
      </>}
